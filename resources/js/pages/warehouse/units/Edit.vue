...
import { index } from '@/actions/App/Http/Controllers/Warehouse/UnitController';
...
const form = useForm({
    ...
    is_active: Boolean(props.unit.is_active ?? true),
});
function submit() {
    form.put(`/warehouse/units/${props.unit.id}`);
}
</script>

<template>
    <Head :title="t('units.editUnit')" />
