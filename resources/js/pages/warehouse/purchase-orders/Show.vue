...
function doReceive() {
    if (confirm(t('purchaseOrders.receive') + '?'))
        router.get(receive.url({ purchase_order: props.order.id }));
}
</script>
        ...
        <AppPageContent>
                ...
                >
                    <div>
                        ...
                        <p class="text-sm text-muted-foreground">
                            ...
                            {{ (props.order.warehouse as any)?.[locale] }}
                        </p>
                    </div>
            ...
            <div class="p-4 pt-4 md:p-6">
                <Table>
                    ...
                    <TableBody>
                        <TableRow
                            v-for="item in (props.order as any).items || []"
                            :key="(item as any).id"
                            class="border-transparent hover:bg-transparent"
                        >
                            <TableCell>{{
                                (item as any).product?.[locale] ?? '-'
                            }}</TableCell>
                ...
                <p class="mt-4 font-medium">
                    {{ t('purchaseOrders.total') }}: {{ props.order.total }}
                </p>
            </div>
        </AppPageContent>
    </AppLayout>
