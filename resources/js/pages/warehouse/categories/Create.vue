...
import { index, store } from '@/actions/App/Http/Controllers/Warehouse/ProductCategoryController';
...
const { t } = useI18n();
const breadcrumbs: BreadcrumbItem[] = [{ title: t('nav.categories'), href: index.url() }, { title: t('categories.createCategory') }];
const form = useForm({ name_tr: '', name_en: '', description_tr: '', description_en: '', parent_id: null as number | null, is_active: true });
function submit() {
    form.post(store.url());
}
</script>
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex flex-1 flex-col gap-4 p-4 md:p-6">
            <Card>
                <CardHeader><CardTitle>{{ t('categories.createCategory') }}</CardTitle><CardDescription>{{ t('categories.title') }}</CardDescription></CardHeader>
                <CardContent>
