        ...
        <div class="flex flex-1 flex-col gap-4 p-4 md:p-6">
            <Card>
                <CardHeader class="flex flex-row items-center justify-between">
                    ...
                    <Link v-if="can('categories.create')" :href="create.url()">
                ...
                <CardContent>
                    <Table>
                        ...
                        <TableBody>
                            <TableRow v-for="c in categories.data" :key="(c as any).id">
                                <TableCell class="font-medium">{{ (c as any)[locale] }}</TableCell>
                                <TableCell>{{ (c as any).products_count ?? 0 }}</TableCell>
                                <TableCell>
                                    <Badge :variant="(c as any).is_active ? 'default' : 'secondary'">
                                        {{ (c as any).is_active ? t('common.active') : t('common.inactive') }}
                                    </Badge>
                                </TableCell>
                                <TableCell>
                                    <DropdownMenu>
                                        ...
                                        <DropdownMenuContent align="end">
                                            <DropdownMenuItem v-if="can('categories.edit')" as-child>
                                                <Link :href="`/warehouse/categories/${(c as any).id}/edit`"><Pencil class="mr-2 h-4 w-4" />{{ t('common.edit') }}</Link>
                                            </DropdownMenuItem>
                                            <DropdownMenuItem v-if="can('categories.delete')" class="text-destructive" @click="destroy((c as any).id)">
                    ...
                    <div v-if="categories.links?.length" class="mt-4 flex gap-2">
                        <Link v-for="(link, i) in categories.links" :key="i" :href="link.url || '#'" class="rounded border px-3 py-1 text-sm" :class="{ 'opacity-50 pointer-events-none': !link.url }">{{ link.label }}</Link>
                    </div>
                </CardContent>
            </Card>
        </div>
