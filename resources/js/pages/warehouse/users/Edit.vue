...
import { index, update } from '@/actions/UserController';
            ...
            <Card>
                <CardContent class="pt-6">
                    <form @submit.prevent="submit" class="grid gap-4 md:grid-cols-2">
                        ...
                        <div class="space-y-2"><Label for="password">{{ t('auth.password') }}</Label><Input id="password" v-model="form.password" type="password" :placeholder="t('users.leaveBlank')" /></div>
                        <div class="space-y-2"><Label for="password_confirmation">{{ t('users.confirmPassword') }}</Label><Input id="password_confirmation" v-model="form.password_confirmation" type="password" /></div>
