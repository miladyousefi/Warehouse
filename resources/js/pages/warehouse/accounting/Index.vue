...
import { DollarSign, Download, MoreHorizontal, Plus, TrendingDown, TrendingUp, Wallet } from 'lucide-vue-next';
import { ref } from 'vue';
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="mx-auto flex h-full w-full max-w-7xl flex-1 flex-col gap-4 p-4 md:p-6">
            <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                <Card>
                    ...
                    <CardContent>
                        <p class="text-2xl font-bold text-emerald-600">{{ Number(income).toLocaleString(undefined, { minimumFractionDigits: 2 }) }}</p>
                    </CardContent>
                </Card>
                ...
                <Card>
                    ...
                    <CardContent>
                        <p class="text-2xl font-bold text-rose-600">{{ Number(expenses).toLocaleString(undefined, { minimumFractionDigits: 2 }) }}</p>
                    </CardContent>
                </Card>
                ...
                <Card>
                    <CardHeader class="flex flex-row items-center justify-between pb-2">
                        <CardTitle class="text-sm font-medium">{{ t('accounting.walletBalance') }}</CardTitle>
                        <Wallet class="h-4 w-4 text-muted-foreground" />
                    </CardHeader>
                    <CardContent>
                        <p :class="`text-2xl font-bold ${walletBalance >= 0 ? 'text-emerald-600' : 'text-rose-600'}`">
            ...
            <div class="grid gap-4 lg:grid-cols-3">
                <Card class="lg:col-span-2">
                    ...
                    <CardContent>
                        <div class="grid gap-3 md:grid-cols-4">
                            <div class="space-y-2">
                                <Label for="year_filter">{{ locale === 'tr' ? 'Yıl' : 'Year' }}</Label>
                                <select id="year_filter" v-model="filterYear" class="w-full rounded-md border border-input bg-background px-3 py-2 text-sm">
                                    <option value="">{{ locale === 'tr' ? 'Tüm Yıllar' : 'All Years' }}</option>
                                    <option v-for="y in years" :key="y" :value="y">{{ y }}</option>
                                </select>
                            </div>
                            <div class="space-y-2">
                                <Label for="month_filter">{{ locale === 'tr' ? 'Ay' : 'Month' }}</Label>
                            ...
                            <div class="md:col-span-4 flex justify-end">
                                <Button size="sm" @click="applyFilters">{{ locale === 'tr' ? 'Filtrele' : 'Apply Filters' }}</Button>
                            </div>
                        </div>
                    </CardContent>
                </Card>
                ...
                <Card>
                    <CardHeader>
                        <CardTitle class="text-base">{{ t('accounting.addTransaction') || (locale === 'tr' ? 'Hızlı İşlem Ekle' : 'Quick Transaction') }}</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <div class="space-y-3">
                            <div class="grid grid-cols-2 gap-2">
                                <Button :variant="walletType === 'input' ? 'default' : 'outline'" size="sm" @click="walletType = 'input'">
                                    {{ t('accounting.walletInput') }}
                                </Button>
                                <Button :variant="walletType === 'output' ? 'default' : 'outline'" size="sm" @click="walletType = 'output'">
                                    {{ t('accounting.walletOutput') }}
                                </Button>
                            </div>
                            <div class="space-y-2">
                                <Label for="wallet_amount">{{ t('common.amount') }}</Label>
                                <Input id="wallet_amount" v-model="walletAmount" type="number" step="0.01" placeholder="0.00" />
                            </div>
                            <div class="space-y-2">
                                <Label for="wallet_description">{{ t('common.description') }}</Label>
                                <Input id="wallet_description" v-model="walletDescription" :placeholder="t('accounting.descriptionPlaceholder') || 'Note'" />
                            </div>
                            <div class="rounded-md border p-2 text-xs text-muted-foreground">
                                {{ t('accounting.walletInput') }}: {{ Number(walletInput).toLocaleString(undefined, { minimumFractionDigits: 2 }) }}
                                <br>
                                {{ t('accounting.walletOutput') }}: {{ Number(walletOutput).toLocaleString(undefined, { minimumFractionDigits: 2 }) }}
                                <br>
                                {{ t('accounting.stockValuation') }}: {{ Number(stockValuation).toLocaleString(undefined, { minimumFractionDigits: 2 }) }}
                            </div>
                            <Button class="w-full" :disabled="isSubmitting || !walletAmount || !walletDescription" @click="addWalletTransaction">
