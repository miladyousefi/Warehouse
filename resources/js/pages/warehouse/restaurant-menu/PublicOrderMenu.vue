...
const total = computed(() =>
    cartItems.value.reduce(
        (sum, x) => sum + Number(x.item.sale_price) * x.quantity,
        0,
    ),
);
...
function submitConfirmedOrder(): void {
    ...
    orderForm.post(
        route('restaurant-order.store', { token: props.table.qr_token }),
        {
            preserveScroll: true,
            onSuccess: () => {
...
function callWaiter(): void {
    waiterForm.post(
        route('restaurant-order.call-waiter', { token: props.table.qr_token }),
        {
            preserveScroll: true,
            onSuccess: () => {
                waiterForm.note = '';
...
<template>
    <Head
        :title="`${t('restaurantMenu.title')} - ${table.name || table.table_number}`"
    />

    <div class="min-h-screen bg-slate-50 text-slate-900">
        <div class="mx-auto max-w-7xl px-4 py-8 md:px-6">
            <section
                class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm md:p-8"
            >
                <div
                ...
                <h1
                    class="mt-2 font-serif text-3xl font-black tracking-tight md:text-4xl"
                >
            ...
            <div class="mt-6 grid gap-6 xl:grid-cols-[1fr_360px]">
                <section class="space-y-6">
                    <article
                        ...
                        :key="cat.id"
                        class="menu-fade space-y-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"
                    ...
                    >
                        <div class="flex items-center gap-3">
                            ...
                            <div
                                v-else
                                class="flex h-10 w-10 items-center justify-center rounded-lg border border-slate-200 bg-slate-50 text-xl"
                            >
                                {{ cat.icon || '🍽️' }}
                            </div>
                        ...
                        <div class="grid gap-3 sm:grid-cols-2">
                            <article
                                ...
                                :key="item.id"
                                class="menu-card rounded-xl border border-slate-200 bg-slate-50 p-3 transition hover:border-slate-300"
                            >
                                <div class="flex gap-3">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        <div class="flex justify-between gap-2">
                                            ...
                                            <strong
                                                class="shrink-0 rounded-full border border-slate-300 bg-white px-2 py-0.5"
                                                >{{
                                                    Number(
                                                        item.sale_price,
                                                    ).toFixed(2)
                    ...
                    >
                        ...
                        <ul v-else class="mt-2 space-y-2 text-sm">
                            ...
                            >
                                <span class="truncate"
                                    >{{
                                        line.item[locale] || line.item.name_tr
                                    }}
                                    x{{ line.quantity }}</span
                                >
                                <span class="shrink-0">{{
                                    (
                                        Number(line.item.sale_price) *
                                        line.quantity
                                    ).toFixed(2)
                                }}</span>
                            </li>
                        ...
                        <div
                            class="mt-3 border-t border-slate-200 pt-3 text-lg font-bold"
                        >
                            {{ t('common.total') }}: {{ total.toFixed(2) }}
                        </div>
                        <p class="mt-1 text-xs text-slate-500">
                            {{ t('restaurantMenu.unpaidInfo') }}
                        </p>
                        <Input
                            v-model="orderForm.customer_note"
                            class="mt-3"
                            :placeholder="t('restaurantMenu.customerNote')"
                        />
                        <Button
                            class="mt-3 w-full"
                            :disabled="
                                orderForm.processing || cartItems.length === 0
                            "
                            @click="placeOrder"
                            >{{ t('restaurantMenu.placeOrder') }}</Button
                        >
                    </section>
                    ...
                    >
                        ...
                        >
                            <BellRing class="h-4 w-4" />{{
                                t('restaurantMenu.callWaiter')
                            }}
                        </h3>
                        <Input
                            v-model="waiterForm.note"
                            class="mt-2"
                            :placeholder="t('restaurantMenu.waiterNote')"
                        />
                        <Button
                            class="mt-3 w-full"
                            variant="outline"
                            :disabled="waiterForm.processing"
                            @click="callWaiter"
                            >{{ t('restaurantMenu.callWaiterButton') }}</Button
                        >
                    </section>
                </aside>
            </div>
        ...
        <div
            v-if="confirmOrderOpen.value"
            class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4"
        >
            <div
                class="w-full max-w-md rounded-xl border border-slate-200 bg-white p-5 shadow-lg"
            >
                <h3 class="text-lg font-semibold">
                    {{ t('restaurantMenu.confirmPlaceOrderTitle') }}
                </h3>
                <p class="mt-2 text-sm text-slate-600">
                ...
                <div class="mt-4 flex justify-end gap-2">
                    <Button
                        variant="outline"
                        size="sm"
                        @click="confirmOrderOpen.value = false"
                        >{{ t('common.cancel') }}</Button
                    >
                    <Button
                        size="sm"
                        :disabled="orderForm.processing"
                        @click="submitConfirmedOrder"
                        >{{
                            t('restaurantMenu.confirmPlaceOrderButton')
                        }}</Button
                    >
                </div>
            </div>
