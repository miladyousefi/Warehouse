...
const form = useForm({
    restaurant_menu_category_id: props.item.restaurant_menu_category_id
        ? String(props.item.restaurant_menu_category_id)
        : '',
    ...
    ingredients: (
        (props.item.ingredients as Array<Record<string, any>> | undefined) ?? []
    ).map((ing) => ({
...
const existingUrls =
    ((props.item.image_gallery_urls as string[] | undefined) ?? []).length > 0
        ? ((props.item.image_gallery_urls as string[] | undefined) ?? [])
        : props.item.image_url
          ? [String(props.item.image_url)]
          : [];
...
function submit() {
    form.patch(`/warehouse/restaurant-menu/${props.item.id}`, {
        forceFormData: true,
    });
}
...
function setImages(event: Event) {
    ...
    if (
        ...
        (existingPreviews.length > 0 || newPreviews.value.length > 0)
    ) {
        form.cover_image_key =
            existingPreviews[0]?.key || newPreviews.value[0].key;
    }
}
</script>
                        ...
                        <div class="space-y-2 md:col-span-2">
                            ...
                            <label class="flex items-center gap-2 text-sm"
                                ><input
                                    ...
                                    type="checkbox"
                                />{{
                                    t('restaurantMenu.removeFoodImage')
                                }}</label
                            >
                        </div>
