...
const topStats = computed(() => [
    {
        key: 'pending',
        icon: ClipboardList,
        label: t('restaurantMenu.orderStatus'),
        value: props.stats.pending_orders,
    },
    {
        key: 'unpaid',
        icon: CreditCard,
        label: t('restaurantMenu.paymentStatus'),
        value: props.stats.unpaid_orders,
    },
    {
        key: 'calls',
        icon: BellRing,
        label: t('restaurantMenu.pendingCalls'),
        value: props.stats.pending_calls,
    },
]);
...
function updateOrder(order: Record<string, any>) {
    ...
    statusForm.cancel_reason =
        order.status === 'cancelled'
            ? String(order.cancel_reason || 'other')
            : null;
    statusForm.patch(
        route('warehouse.restaurant-orders.update-status', { order: order.id }),
    );
}

function markCallHandled(callId: number) {
    router.patch(
        route('warehouse.restaurant-orders.calls.handled', { call: callId }),
    );
}
...
function saveTable(tableId: number) {
    tableEditForm.patch(
        route('warehouse.restaurant-orders.tables.update', { table: tableId }),
        {
            preserveScroll: true,
            onSuccess: () => {
                editingTableId.value = null;
...
function regenerateTableLink(tableId: number) {
    router.patch(
        route('warehouse.restaurant-orders.tables.regenerate-link', {
            table: tableId,
        }),
        {},
...
function deleteTable(tableId: number) {
    ...
    router.delete(
        route('warehouse.restaurant-orders.tables.destroy', { table: tableId }),
        {
            preserveScroll: true,
...
function refreshBoard() {
    router.reload({
        only: ['orders', 'calls', 'stats', 'tables'],
        preserveScroll: true,
    });
}
        ...
        <AppPageContent>
            <template #header>
                <div class="p-4 pb-0 md:p-6 md:pb-0">
                    <h1 class="text-xl font-semibold">
                        {{ t('nav.restaurantOrders') }}
                    </h1>
                    <p class="text-sm text-muted-foreground">
                        {{ t('restaurantMenu.liveBoardInfo') }}
                    </p>
                </div>
            </template>
            <div class="space-y-4 p-4 pt-4 md:p-6 md:pt-4">
                <div class="flex flex-wrap gap-2">
                    ...
                    <Link
                        v-if="can('restaurant_orders.take_order')"
                        :href="manualOrderCreateUrl"
                    >
                        <Button>
                ...
                <Card :class="glassCardClass">
                    ...
                    <CardContent>
                        ...
                        >
                            {{ t('restaurantMenu.noPendingCalls') }}
                        </div>
                    ...
                    <CardContent>
                        <Table class="rounded-md border border-amber-200/40">
                            <TableHeader>
                                <TableRow>
                                    <TableHead>{{
                                        t('restaurantMenu.orderCode')
                                    }}</TableHead>
                                    <TableHead>{{
                                        t('restaurantMenu.tableLabel')
                                    }}</TableHead>
                                    <TableHead>{{
                                        t('restaurantMenu.orderStatus')
                                    }}</TableHead>
                                    <TableHead>{{
                                        t('restaurantMenu.paymentStatus')
                                    }}</TableHead>
                                ...
                                >
                                    ...
                                    <TableCell class="text-right">
                                        <div class="flex justify-end gap-2">
                                            <Link
                                                :href="`/warehouse/restaurant-orders/${order.id}`"
                                            >
                                                <Button
                                                    size="icon-sm"
                                                    variant="outline"
                    ...
                    <CardContent class="space-y-4">
                        ...
                        >
                            ...
                            <div class="grid gap-2 md:grid-cols-6">
                                <Input
                                    v-model="tableCreateForm.table_number"
                                    :placeholder="
                                        t('restaurantMenu.tableNumber')
                                    "
                                    class="md:col-span-1"
                                />
                                <Input
                                    v-model="tableCreateForm.name"
                                    :placeholder="t('common.name')"
                                    class="md:col-span-2"
                                />
                                <Input
                                    v-model="tableCreateForm.capacity"
                                    type="number"
                                    min="1"
                                    :placeholder="t('restaurantMenu.capacity')"
                                    class="md:col-span-1"
                                />
                                <Input
                                    v-model="tableCreateForm.section"
                                    :placeholder="t('restaurantMenu.section')"
                                    class="md:col-span-1"
                                />
                                <Button
                                    class="md:col-span-1"
                                    :disabled="tableCreateForm.processing"
                                    @click="createTable"
                                    >{{ t('common.add') }}</Button
                                >
                            </div>
                            ...
                            >
                                <input
                                    v-model="tableCreateForm.is_active"
                                    type="checkbox"
                                />
                        ...
                        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                            ...
                            >
                                <template v-if="editingTableId === table.id">
                                    <div class="space-y-2">
                                        <Input
                                            v-model="tableEditForm.table_number"
                                            :placeholder="
                                                t('restaurantMenu.tableNumber')
                                            "
                                        />
                                        <Input
                                            v-model="tableEditForm.name"
                                            :placeholder="t('common.name')"
                                        />
                                        <Input
                                            v-model="tableEditForm.capacity"
                                            type="number"
                                            min="1"
                                            :placeholder="
                                                t('restaurantMenu.capacity')
                                            "
                                        />
                                        <Input
                                            v-model="tableEditForm.section"
                                            :placeholder="
                                                t('restaurantMenu.section')
                                            "
                                        />
                                        <label
                                        ...
                                        <div class="flex gap-2">
                                            <Button
                                                ...
                                                class="flex-1"
                                                :disabled="
                                                    tableEditForm.processing
                                                "
                                                @click="saveTable(table.id)"
                                ...
                                <template v-else>
                                    ...
                                    <div
                                        class="mt-2 flex items-center gap-2 overflow-x-auto whitespace-nowrap"
                                    >
                                        <Button
                                            size="icon-sm"
                                            variant="outline"
                                            :title="
                                                t('restaurantMenu.copyLink')
                                            "
                                            @click="
                                                copyTableUrl(table.order_url)
                                            "
                                        >
                                            <Copy class="h-4 w-4" />
                                            <span class="sr-only">{{
                                        ...
                                        <Button
                                            ...
                                            :title="
                                                t(
                                                    'restaurantMenu.regenerateLink',
                                                )
                                            "
                                            @click="
                                                regenerateTableLink(table.id)
                                            "
                                        >
