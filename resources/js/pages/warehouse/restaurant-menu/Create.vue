...
const props = defineProps<{ categories: Array<Record<string, any>>; products: Array<Record<string, any>> }>();
...
function syncImagesToForm() {
  ...
  const hasSelectedCover = imagePreviews.value.some((x) => x.key === form.cover_image_key);
  if (!hasSelectedCover) {
...
function removeImage(index: number) {
  ...
  imagePreviews.value = imagePreviews.value.map((img, idx) => ({ ...img, key: `new:${idx}` }));
  syncImagesToForm();
}
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="flex h-full flex-1 flex-col gap-6 p-4 md:p-6">
      <section class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        <h1 class="text-2xl font-black tracking-tight text-slate-900 md:text-3xl">{{ t('restaurantMenu.addFood') }}</h1>
      ...
      <form class="grid gap-4 xl:grid-cols-[1fr_340px]" @submit.prevent="submit">
        <Card>
          <CardHeader><CardTitle>{{ t('restaurantMenu.addFood') }}</CardTitle></CardHeader>
          <CardContent class="grid gap-4 md:grid-cols-2">
            ...
            <div class="space-y-2 md:col-span-2">
              ...
              <div
                ...
                :class="isDragging ? 'border-primary bg-primary/5' : 'border-slate-300 bg-slate-50'"
                @click="openFilePicker"
                @dragover.prevent="isDragging = true"
                @dragleave.prevent="isDragging = false"
                @drop="onDrop"
              >
                <Upload class="mx-auto mb-2 h-7 w-7 text-slate-500" />
                <p class="text-sm font-semibold text-slate-800">{{ t('restaurantMenu.dragDropImages') || 'Drag images here or click to upload' }}</p>
                <p class="text-xs text-slate-500">JPG, PNG, WEBP</p>
              </div>
              <p v-if="form.errors.images" class="text-xs text-destructive">{{ form.errors.images }}</p>
              <p v-if="(form.errors as any)['images.0']" class="text-xs text-destructive">{{ (form.errors as any)['images.0'] }}</p>

              <div class="grid gap-3 md:grid-cols-4">
                <div v-for="(img, index) in imagePreviews" :key="img.key" class="rounded-xl border border-slate-200 bg-white p-2 text-xs shadow-sm">
                  <img :src="img.url" alt="food" class="mb-2 h-28 w-full rounded-lg object-cover" />
                  <div class="flex items-center justify-between gap-2">
                    <label class="flex items-center gap-2 text-slate-700">
                      <input v-model="form.cover_image_key" :value="img.key" type="radio" name="cover_create" />
                      {{ t('restaurantMenu.selectCover') }}
                    </label>
                    <button type="button" class="rounded p-1 hover:bg-slate-100" @click="removeImage(index)">
                      <X class="h-3.5 w-3.5 text-slate-600" />
                    </button>
                  </div>
                </div>
              </div>
        ...
        <div class="space-y-4 xl:sticky xl:top-4 xl:self-start">
          ...
          <Card>
            <CardHeader><CardTitle>{{ t('restaurantMenu.selectCover') }}</CardTitle></CardHeader>
            <CardContent>
              <div class="text-sm text-slate-600">
                {{ imagePreviews.length > 0 ? `${imagePreviews.length} images uploaded` : t('restaurantMenu.foodImages') }}
              </div>
            </CardContent>
          </Card>
        </div>
