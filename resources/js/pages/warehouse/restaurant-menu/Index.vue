    ...
    <AppPageContent>
      <template #header>
        <div class="p-4 pb-0 md:p-6 md:pb-0">
          <div>
            <h1 class="text-xl font-semibold">{{ t('restaurantMenu.title') }}</h1>
            <p class="text-sm text-muted-foreground">{{ t('restaurantMenu.menuSubtitle') }}</p>
          </div>
        </div>
      </template>
      <div class="space-y-4 p-4 pt-4 md:p-6 md:pt-4">
          ...
          <Card class="xl:col-span-3">
            ...
            <CardContent class="space-y-4">
              ...
              <div class="rounded-md border p-3 text-xs">
                ...
                <div class="break-all text-muted-foreground">{{ shareUrl || '-' }}</div>
        ...
        <Card>
          ...
          <CardContent class="space-y-3 md:hidden">
            <article v-for="item in items.data" :key="item.id" class="rounded-md border p-3">
              <div class="flex items-start justify-between gap-3">
                <div class="flex min-w-0 items-center gap-2">
                  <img v-if="item.image_url" :src="item.image_url" class="h-10 w-10 rounded object-cover" alt="food" />
                  <div class="min-w-0">
                    <h3 class="truncate font-semibold">{{ item[locale] || item.name_tr }}</h3>
                    <p class="truncate text-xs text-muted-foreground">{{ item.category?.[locale] || '-' }}</p>
                  </div>
                </div>
                <strong>{{ Number(item.sale_price).toFixed(2) }}</strong>
              </div>
              <div class="mt-2 grid grid-cols-2 gap-2 text-xs">
                <div class="rounded-md bg-muted p-2">{{ t('restaurantMenu.foodCost') }}: {{ Number(item.food_cost || 0).toFixed(2) }}</div>
                <div class="rounded-md bg-muted p-2">{{ t('restaurantMenu.profit') }}: {{ Number(item.profit || 0).toFixed(2) }}</div>
              </div>
