...
function setupListener(): void {
    ...
    w.Echo.private('restaurant-calls').listen('.waiter.called', (payload: any) => {
        openCall(payload ?? {});
    });
}
...
<template>
    <div v-if="show && canSeeCalls" class="fixed inset-0 z-[120] flex items-center justify-center bg-black/35 p-4">
            ...
            <div class="flex items-start gap-3">
                ...
                <div class="min-w-0">
                    ...
                    <p class="mt-1 text-sm text-muted-foreground">
                        {{ tableLabel ? `Table: ${tableLabel}` : 'A table called waiter.' }}
                    </p>
                    <p v-if="note" class="mt-1 text-xs text-muted-foreground">Note: {{ note }}</p>
                </div>
            </div>
